test_that("calc_p_value_multi() works for comb = TRUE", {
  ...
  dat <- data.frame(lineup_id = c(1, 1, 2),
                    ...
                    n_sel = c(1, 1, 2))
  result <- visage::calc_p_value_multi(dat, comb = TRUE, n_eval = 1:2)
...
test_that("calc_p_value_multi() works for comb = FALSE", {
  ...
  dat <- data.frame(lineup_id = c(1, 1, 2),
                    detected = c(TRUE, FALSE, TRUE),
                    n_sel = c(1, 1, 2))
  result <- visage::calc_p_value_multi(dat, comb = FALSE, n_eval = 1:2)
...
test_that("calc_p_value_multi() supports cache", {
  dat <- data.frame(lineup_id = c(1, 1, 2),
                    ...
                    n_sel = c(1, 1, 2))
  e <- new.env()
  result <- visage::calc_p_value_multi(dat, comb = TRUE, n_eval = 1:2, cache_env = e)
  result1 <- visage::calc_p_value_multi(dat, comb = TRUE, n_eval = 1:2, cache_env = e)
