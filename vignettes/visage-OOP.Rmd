...
This gives us the ability to define hierarchical class system. 
...
There is nothing special about `self`. It is just a conventional name used in Python inside the body of a method to refer to an object itself. 
...
To define a new class, we need to use the function `new_class()` with a class name passed to the `class_name` argument. This new class will automatically inherit from `BASE` to provide some basic attributes and methods. If you would like to write your own base object classes, you need to set `empty_class = TRUE`.
...
## Attributes and methods provided by `BASE`

Let's explore the structure of the `DEMO` class with `lobstr::tree`. You may notice from the output that `DEMO` contains many attributes and methods. 
...
- `..type..` represents the current class name.

```{r}
DEMO$..type..
```

- `..class..` contains a character vector of class names, including the current class name and inherited class names.

```{r}
DEMO$..class..
```

- `..instantiated..` represents whether this environment is an instance or not.

```{r}
DEMO$..instantiated..
```

These attributes should not be modified manually. The OOP system will manage their values.

### Methods

The `BASE` class provides some very useful methods. 

- `..dict..()` returns all attribute and method names in the environment.

```{r}
DEMO$..dict..()
```

- `..str..()` is called by the S3 method `print.visage_oop()` to give a nicely formatted string representation of the object. This is similar to Python's `__str__()` dunder method.

```{r}
DEMO$..str..()
```

- `..methods..()` returns all method names in the environment

```{r}
DEMO$..methods..()
```

- `..len..()` returns the length of the object. This method is undefined in the `BASE` class, but will be useful in subclasses where "length" is meaningful. It is similar to `__len__` in Python.

- `..init..()` is the initializer, which is also undefined in the `BASE` class. It is similar to `__init__` in Python.

- `..new..()` is the constructor. It will try to build an instance from the class without running `..init..()`. This is different from Python as the default `__new__` method will invoke `__init__` before returning to the caller.

- `..repr..()` is the "official" string representation of an object. It tries to catch the expression which builds the instance. 

- `instantiation()` builds the instance and runs the initializer. This is equivalent to `__new__` in Python. 

```{r}
DEMO$instantiation()
```

Usually, we can bind this method to a function for convenience.

```{r}
demo_ <- DEMO$instantiation
demo_()
```

## Define a method

Method can be defined via `register_method()`. Both inline and pre-defined functions are accepted. The syntax is `register_method(object, method_name_1 = function_1, method_name_2 = function_2)`.

```{r}
myfunc <- function() 2
register_method(DEMO, a = function() 1, b = myfunc)
...
In python, `self` needs to be passed as the first argument of method. In `visage`, `self` doesn't need to be passed and can be directly used inside the method body. 

```{r}
DEMO$x <- 1
register_method(DEMO, x_plus_one = function() self$x + 1)

DEMO$x_plus_one()
```

Note that, `self` points to the object itself.

```{r}
register_method(DEMO, str_self = function() str(self))
str(DEMO)
DEMO$str_self()
```
...
Additionally, variables stored in the object will not expose to methods, but variables stored in the parent environment of the object will. This is similar to the Python LEGB rule.

```{r}
DEMO$x <- 1
register_method(DEMO, try_x = function() x)
try(DEMO$try_x())
